...
2. Redis 설정
Redis 서버가 실행 중이어야 합니다. .env 설정이 없다면 기본적으로 localhost:6379에 접속을 시도합니다. (필요시 redis.service.ts의 process.env.redis_host, redis_port 환경변수 설정)
...
서버는 Redis ZADD queue:concert_2025 {timestamp} {userId} 실행.

유저는 '대기 중' 상태가 되며 내 순번을 계속 확인(Polling).
